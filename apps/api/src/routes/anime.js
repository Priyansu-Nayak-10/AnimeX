...
const { apiResponse, apiError, processAnimeList } = require('../utils/helpers');
...
const searchValidator = validate({
    query: {
        q: { type: 'string', required: false, maxLength: 120 },
        page: { type: 'number', min: 1, max: 200 },
        limit: { type: 'number', min: 1, max: 25 },
    },
});

router.get('/search', checkCache(TTL_12H), searchValidator, async (req, res) => {
    try {
        ...
        const year = toPositiveInt(req.query.year, 0);
        ...
        return apiResponse(res, normalizedData, 200, 'Search results', { pagination: data.pagination });
    } catch (err) {
