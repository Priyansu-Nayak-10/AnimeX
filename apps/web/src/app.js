...
import { createDataStore } from './core/state.js';
import * as selectors from './core/state.js';
import { initLibraryCloudSync } from './core/cloudSync.js';
...
async function bootstrap() {
  ...
  const modules = [
    ...
    initLibraryCloudSync({ libraryStore, toast }),
    initSectionReveal()
  ];
  ...
  if (user?.id) {
    try {
      const { syncService } = await import('./core/syncService.js');
