...
import { resolveEpisodes, resolveEpisodesNumeric } from "../../core/dataNormalize.js";
...
function initWatchlistBoard({ libraryStore, toast = null }) {
  ...
  function getRows() {
    return applyTypeAndSort(getWatchlistCandidates(), uiState.typeFilter, uiState.sortAsc);
  }
  ...
  function buildToolbar() {
    ...
    return `<div class="watchlist-command-bar" data-watchlist-toolbar="1">
      ...
      <div class="watchlist-controls-group">
        <button class="wl-control-btn" data-watchlist-action="toggle-az">A-Z ${isAsc ? "↑" : "↓"}</button>
  ...
  function buildWatchlistCard(item, nextStatusMemory) {
    ...
    const statusAnimateClass = previousStatus && previousStatus !== item.status ? "status-change" : "";
    nextStatusMemory.set(item.malId, item.status);
    return `<article class="wl-card-vertical watching-card-premium watchlist-item status-${escapeHtml(item.status)}" draggable="true" data-action="open-anime-modal" data-id="${Number(item?.malId || 0)}"><div class="wl-card-media"><img src="${escapeHtml(item?.image || "")}" alt="${escapeHtml(normalizeTitle(item))}" class="wl-card-poster" loading="lazy"><div class="wl-overlay"></div><div class="wl-status-badge ${escapeHtml(item.status)} ${statusAnimateClass}">${escapeHtml(item.status)}</div><div class="wl-progress-overlay"><div class="wl-progress-bar"><div class="wl-progress-fill progress-glow" style="width:${percent}%"></div></div><span class="wl-progress-text-overlay">${progress} / ${episodeDisplay}</span></div></div><div class="wl-card-content"><h3 class="wl-card-title">${escapeHtml(normalizeTitle(item))}</h3><p class="wl-card-meta">${genreText} | ${yearText}</p></div></article>`;
  }
  ...
  function render() {
    ...
    if (rows.length <= WATCHLIST_LARGE_LIST_THRESHOLD) {
      watchlistBoard.innerHTML = `<div class="watchlist-grid">${rows.map((item) => buildWatchlistCard(item, nextStatusMemory)).join("")}</div>`;
    ...
    watchlistBoard.innerHTML = '<div class="watchlist-grid" data-watchlist-grid="1"></div>';
    ...
    function pump() {
      ...
      if (slice.length) grid.insertAdjacentHTML("beforeend", slice.map((item) => buildWatchlistCard(item, nextStatusMemory)).join(""));
      cursor += slice.length;
...
function initCompletedBoard({ libraryStore, toast = null }) {
  ...
  function buildCompletedCard(item) {
    ...
    return `
      <div class="trophy-card-wrapper premium-card-wrapper">
        <div class="premium-cover-card" data-id="${malId}">
          ...
          <div class="cover-actions single">
            <button class="status-pill active" data-completed-action="rewatch" data-id="${malId}">Rewatch</button>
          </div>
        </div>
      </div>
    `;
  ...
  function render() {
    ...
    if (rows.length <= COMPLETED_LARGE_LIST_THRESHOLD) {
      completedList.innerHTML = rows.map((item) => buildCompletedCard(item)).join("");
      return;
    }
